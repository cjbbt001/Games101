...
| `Assignment2/Code/` | 光栅化：深度缓冲、MSAA | **是** | OpenCV, Eigen |
| `Assignment3/Code/` | 着色：Blinn-Phong、纹理、法线贴图 | **进行中** | OpenCV, Eigen |
...
- `get_projection_matrix`：透视投影 = 透视挤压 → 正交平移 → 正交缩放
  - 传入正数 zNear/zFar，用 `zNear * tan` 算 t，乘 `M_flip`（z轴翻转）修正方向
  - 三个作业统一此写法
- `get_rotation`：Rodrigues 公式 `R = cosθ I + (1-cosθ) aaᵀ + sinθ N`
- `build/` 已有编译产物，运行：`./Rasterizer -r 20 output.png`

### Assignment2
- 已做：`insideTriangle()`、`rasterize_triangle()`（含 SSAA）
- 深度测试不取负，直接用 viewport 变换后的 z 值（近小远大），配合 `<` 判断
- `build/` 已有编译产物，运行：`./Rasterizer output.png`

### Assignment3（进行中）
- `get_projection_matrix`：已完成，与作业1/2统一写法
- `rasterize_triangle`：框架已写好，无需改动
- 下一步：`phong_fragment_shader`
- 参考代码备份：`Assignment3/hw3_reference_code.md`
